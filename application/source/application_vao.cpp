...
void ApplicationVao::initializeGeometry() {
    ...
    glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, stride,
                          (GLvoid*)uintptr_t(sizeof(float) * 3));

    // generate index buffer and bind to vao
    // index data
    std::vector<std::uint8_t> indices{0, 1, 2};
    ...
    // configure currently bound array buffer
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(std::uint8_t) * indices.size(),
...
void ApplicationVao::render() const {
    ...
    glUniformMatrix4fv(m_shaders.at("vao").u_locs.at("ModelViewMatrix"), 1,
...
void ApplicationVao::uploadUniforms() {
    ...
    glUniformMatrix4fv(m_shaders.at("vao").u_locs.at("ProjectionMatrix"), 1,
