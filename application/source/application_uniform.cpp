...
void ApplicationUniform::initializeShaderPrograms() {
    ...
    m_shaders.emplace(
        "uniform",
        shader_program{
            {{GL_VERTEX_SHADER, m_resource_path + "shaders/uniform.vert"},
             {GL_FRAGMENT_SHADER,
...
void ApplicationUniform::render() const {
    ...
    glUniformMatrix4fv(m_ul_model_view, 1, GL_FALSE,
...
void ApplicationUniform::uploadUniforms() {
    ...
    m_ul_model_view =
        glGetUniformLocation(m_shaders.at("uniform").handle, "ModelViewMatrix");
    m_ul_projection = glGetUniformLocation(m_shaders.at("uniform").handle,
                                           "ProjectionMatrix");
    // upload matrix to gpu
    glUniformMatrix4fv(m_ul_projection, 1, GL_FALSE,
