...
pixel_data file(std::string const& file_name) {
    ...
    data_ptr =
        stbi_load(file_name.c_str(), &width, &height, &format, STBI_rgb_alpha);

    if (!data_ptr) {
        throw std::logic_error(std::string{"stb_image: "} +
                               stbi_failure_reason());
    }
    ...
    if (format == STBI_grey) {
        ...
        throw std::logic_error(
            "stb_image: misinterpreted data, incorrect format");
    }
    ...
    return pixel_data{texture_data, pixel_format, GL_UNSIGNED_BYTE,
