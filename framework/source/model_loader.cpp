...
void generate_normals(tinyobj::mesh_t& model) {
    ...
    for (unsigned i = 0; i < model.indices.size(); i += 3) {
        glm::fvec3 normal = glm::cross(
            positions[model.indices[i + 1]] - positions[model.indices[i]],
            positions[model.indices[i + 2]] - positions[model.indices[i]]);
...
std::vector<glm::fvec3> generate_tangents(tinyobj::mesh_t const& model) {
    ...
    std::vector<glm::fvec3> tangents(model.positions.size() / 3,
                                     glm::fvec3{0.0f});
    ...
    for (unsigned i = 0; i < model.positions.size(); i += 3) {
        positions[i / 3] = glm::fvec3{
            model.positions[i], model.positions[i + 1], model.positions[i + 2]};
        normals[i / 3] = glm::fvec3{model.normals[i], model.normals[i + 1],
                                    model.normals[i + 2]};
    }
    for (unsigned i = 0; i < model.texcoords.size(); i += 2) {
        texcoords[i / 2] =
            glm::fvec2{model.texcoords[i], model.texcoords[i + 1]};
    }
    ...
    for (unsigned i = 0; i < model.indices.size() / 3; i++) {
        ...
        //                        model.indices[i * 3 + 1],
        //                        model.indices[i * 3 + 2]};
        // access an attribute of xth vert with vector access
        // "attribute[indices[x]]"

        // calculate tangent for the triangle and add it to the accumulation
        // tangents of the adjacent vertices see generate_normals() for similar
        // workflow
    }
