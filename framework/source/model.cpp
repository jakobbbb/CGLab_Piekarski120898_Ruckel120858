...
model::attribute const model::INDEX{1 << 5, sizeof(unsigned), 1,
                                    GL_UNSIGNED_INT};

model::model() : data{}, indices{}, offsets{}, vertex_bytes{0}, vertex_num{0} {}

model::model(std::vector<GLfloat> const& databuff,
             attrib_flag_t contained_attributes,
             std::vector<GLuint> const& trianglebuff)
    : data(databuff),
    ...
    for (auto const& supported_attribute : model::VERTEX_ATTRIBS) {
        ...
        if (supported_attribute.flag & contained_attributes) {
            ...
            offsets.insert(std::pair<attrib_flag_t, GLvoid*>{
                supported_attribute, (GLvoid*)uintptr_t(vertex_bytes)});
            // move offset pointer forward
            vertex_bytes +=
                supported_attribute.size * supported_attribute.components;
